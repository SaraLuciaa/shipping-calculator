...
class ShippingQuoteService
{
    ...
    /**
     * Cotiza un producto con detalles completos (peso real, volumétrico, flete, empaque, seguro)
     */
    public function quoteWithDetails($id_product, $id_city, $qty = 1)
    {
        if ((int)$id_city <= 0) {
            return [];
        }
            ...
            try {
                ...
                if ($shippingCost !== null && $shippingCost > 0) {
                    ...
                    $quotes[] = [
                        ...
                        'type' => $type,
                        'price' => $totalPrice,
                        'peso_real' => round($p_weight, 2),
                        'peso_volumetrico' => round($volWeight, 2),
                        'peso_facturable' => round($billable, 2),
    ...
    /**
     * Método quote original (legacy) - redirige a quoteWithDetails
     */
    public function quote($id_product, $id_city, $qty = 1)
    {
        return $this->quoteWithDetails($id_product, $id_city, $qty);
    }
    ...
    public function quoteMultipleWithGrouped(array $items, $id_city)
    {
        ...
        if (!empty($individualNonGroupableItems)) {
            ...
            foreach ($individualNonGroupableItems as $item) {
                ...
                // Para NO agrupables, cotizar cada unidad individualmente
                $quotes = $this->quoteWithDetails($id_product, $id_city, $qty);
    ...
    private function quotGroupedPackages(array $packages, $id_city, $lang_id)
    {
        ...
        foreach ($packages as $package) {
            ...
            // Calcular valor total del paquete y preparar items individuales
            $packageValue = 0.0;
            $totalRealWeight = 0;
            $packageItems = []; // Array de items individuales con dimensiones
            $itemsDetailed = []; // Para mostrar resumen
            
            // Obtener factor volumétrico máximo para cálculo de display
            $maxVolumetricFactor = $this->getMaxVolumetricFactor();
            ...
            foreach ($package['items'] as $item) {
                ...
                $realWeightUnit = (float)$product->weight;
                $height = (float)$product->height;
                $width = (float)$product->width;
                $depth = (float)$product->depth;
                $units = $item['units_in_package'];
                
                $totalRealWeight += $realWeightUnit * $units;
                
                // Calcular peso volumétrico unitario con el factor máximo
                $volumetricWeightUnit = 0;
                if ($height > 0 && $width > 0 && $depth > 0) {
                    $volumetricWeightUnit = $this->weightCalc->volumetricWeight($depth, $width, $height, $maxVolumetricFactor);
                }
                
                // Agregar cada unidad individual al array para cálculo por transportadora
                for ($i = 0; $i < $units; $i++) {
                    $packageItems[] = [
                        'id_product' => $item['id_product'],
                        'height' => $height,
                        'width' => $width,
                        'depth' => $depth,
                        'weight' => $realWeightUnit,
                        'name' => $itemName
                    ];
                }
                
                // Información detallada del item para mostrar
                $itemsDetailed[] = [
                    'id_product' => $item['id_product'],
                    'name' => $itemName,
                    ...
                    'dimensions' => sprintf('%scm × %scm × %scm', $depth, $width, $height),
                    'total_real_weight' => $realWeightUnit * $units,
                    'total_volumetric_weight' => $volumetricWeightUnit * $units,
                    'price_unit' => $price
                ];
            }

            // Cotizar este paquete - cada transportadora calculará con su factor
            $quotes = $this->quoteGroupedPackageDetailed($id_city, $packageValue, $totalRealWeight, $packageItems);
    ...
    /**
     * Cotiza un paquete agrupado calculando detalles por cada transportadora
     * Muestra operaciones detalladas de cálculo de peso volumétrico
     */
    private function quoteGroupedPackageDetailed($id_city, $declaredValue, $totalRealWeight, $packageItems)
    {
        if ((int)$id_city <= 0) {
            return [];
        }
        ...
        $totalRealWeight = (float) $totalRealWeight;
        ...
        foreach ($carriersWithCoverage as $carrier) {
            try {
                ...
                // Calcular peso volumétrico sumando CADA ITEM individual
                ...
                foreach ($packageItems as $item) {
                    ...
                    if ($height > 0 && $width > 0 && $depth > 0) {
                        $volWeightUnit = $this->weightCalc->volumetricWeight($depth, $width, $height, $kgVol['value_number']);
                        $totalVolumetricWeight += $volWeightUnit;
                    }
                }
                
                // Peso facturable es el mayor entre real y volumétrico total
                $billableWeight = max($totalRealWeight, $totalVolumetricWeight);
                ...
                $quotes[] = [
                    'carrier' => $carrier['name'],
                    'type' => $type === 'per_kg' ? 'Por Kg' : 'Por Rangos',
                    'price' => $totalPrice,
                    'peso_real' => round($totalRealWeight, 2),
                    'peso_volumetrico' => round($totalVolumetricWeight, 2),
    ...
    /**
     * Cotiza un peso específico con detalles completos por transportadora
     * Incluye: peso real, peso volumétrico, flete, empaque, seguro, total
     */
    private function quoteByWeightWithDetails($weight, $id_city, $declaredValue = 0.0, $height = 0, $width = 0, $depth = 0, $units = 1, $realWeight = 0)
    {
        if ((int)$id_city <= 0) {
            return [];
        }
        ...
        foreach ($carriersWithCoverage as $carrier) {
            try {
                ...
                // Obtener factor volumétrico de cada transportadora
                $kgVol = Db::getInstance()->getRow("
                    SELECT value_number
                    FROM " . _DB_PREFIX_ . "shipping_config
                    WHERE name = 'Peso volumetrico'
                ...
                // Calcular peso volumétrico específico para esta transportadora
                $volumetricWeight = 0;
                if ($height > 0 && $width > 0 && $depth > 0) {
                    $volumetricWeight = $this->weightCalc->volumetricWeight($depth, $width, $height, $kgVol['value_number']);
                    $volumetricWeight *= $units; // Multiplicar por unidades en el paquete
                }
                
                // Peso facturable es el mayor entre real y volumétrico
                $billableWeight = max($realWeight, $volumetricWeight);
                ...
                $quotes[] = [
                    ...
                    'peso_volumetrico' => round($volumetricWeight, 2),
                    'peso_facturable' => round($billableWeight, 2),
                    'flete' => round($shippingCost, 2),
                    'empaque' => round($packagingCost, 2),
                    'seguro' => round($insuranceCost, 2),
    ...
    /**
     * Cotiza un peso específico (para paquetes agrupados) - Método legacy
     */
     ...
     * Esto asegura que los paquetes respeten el límite de 60 kg en TODAS las transportadoras
     */
    private function getMinVolumetricFactor()
    {
        ...
        return 4000; // Default conservador
    }

    /**
     * Obtiene el factor volumétrico MÁXIMO de todos los carriers (legacy)
     */
    private function getMaxVolumetricFactor()
    {
        ...
        if ($result && isset($result['max_factor']) && $result['max_factor'] > 0) {
            return (int) $result['max_factor'];
        }
